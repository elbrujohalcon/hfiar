...
bestColumnFor p b =
    head $ catMaybes [columnWhereWins p b,
                      columnWhereLoses p b,
                      column3ifAvailable b,
                      firstAvailableColumn b]
...
column3ifAvailable :: [[Tile]] -> Maybe Int
column3ifAvailable b = case length (b !! 3) of
                            7 -> Nothing
                            _ -> Just 3

firstAvailableColumn :: [[Tile]] -> Maybe Int
firstAvailableColumn = Just . length . takeWhile (\c -> length c == 7)
